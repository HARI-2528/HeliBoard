...
//            else Event.createSoftwareKeypressEvent(primaryCode, metaState, mkv.getKeyX(x), mkv.getKeyY(y), isKeyRepeat)
    ...
    private fun handleAiAction(primaryCode: Int) {
        ...
        if (primaryCode == KeyCode.AI_PROOFREAD) {
            ...
            Thread {
                val result = AiHelper.groqRequestStreaming(textBefore + textAfter, latinIME.getString(R.string.ai_proofread_prompt), token, { })
        ...
        val dialog = AlertDialog.Builder(getPlatformDialogThemeContext(latinIME))
            ...
            .setPositiveButton(R.string.ai_go) { d, _ ->
                ...
                if (primaryCode == KeyCode.AI_REPHRASE) {
                    ...
                    connection.selectAll()
                    Thread {
                        val result = AiHelper.groqRequestStreaming(textBefore + textAfter, latinIME.getString(prompt).format(userText), token, { })
                        latinIME.mHandler.post { connection.commitText(result, 1) }
                    }.start()
                } else {
                    Thread {
                        val result = AiHelper.groqRequestStreaming(userText, latinIME.getString(prompt), token, { })
                        latinIME.mHandler.post { connection.commitText("$result ", 1) }
                    }.start()
                }
            }
        ...
        if (action.needsInput) {
            ...
            val dialog = AlertDialog.Builder(getPlatformDialogThemeContext(latinIME))
                ...
                .setPositiveButton(R.string.ai_go) { d, _ ->
                    ...
                    val finalPrompt = if (action.prompt.isBlank()) {
                        userText
                    } else if (action.prompt.contains("%s")) {
                        action.prompt.format(userText)
                    } else {
                        "${action.prompt}\n$userText"
                    }
                    val finalMessage = if (fullText.isBlank()) "Generate output based on the instructions." else fullText
                    if (fullText.isNotBlank()) {
                        connection.selectAll()
                    }
                    Thread {
                        val result = AiHelper.groqRequestStreaming(finalMessage, finalPrompt, token, { })
                        latinIME.mHandler.post { connection.commitText(result, 1) }
                    }.start()
                }
                .setNegativeButton(R.string.ai_cancel, null)
